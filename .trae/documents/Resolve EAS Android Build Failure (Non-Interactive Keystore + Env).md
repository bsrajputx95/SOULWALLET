...
* Env resolution message shows EXPO\_PUBLIC\_\* variables are loaded from eas.json but no plain/sensitive variables are configured on the server; this is not a blocker unless your workflow requires them (e.g., Sentry auth token, private NPM token).
